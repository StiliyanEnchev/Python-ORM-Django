...
def find_books_by_genre_and_language(book_genre, book_language):
    return Book.objects.filter(language=book_language, genre=book_genre)
...
def find_authors_nationalities():
    ...
    result = ''
    for a in authors:
        result += (f"{a.first_name} {a.last_name} is {a.nationality}\n")
    return result
...
def order_books_by_year():
    ...
    for b in books:
        result += f'{b.publication_year} year: {b.title} by {b.author}\n'
    return result
...
def filter_authors_by_nationalities(nationality):
    ...
    for a in authors:
        result += f'{a.biography}\n' if a.biography is not None else f'{a.first_name} {a.last_name}\n'
    return result
...
def change_reviewer_name(reviews_name, new_name):
    Review.objects.filter(reviewer_name=reviews_name).update(reviewer_name=new_name)
